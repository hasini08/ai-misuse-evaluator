...
def validate_input(df: pd.DataFrame) -> None:
    ...
    if "transparency_score" in df.columns:
        values = df["transparency_score"].astype(float)
        if ((values < 1) | (values > 5)).any():
            raise ValueError("transparency_score must be within 1-5")
...
def compute_scores(
    ...
    w_transparency: float = DEFAULT_WEIGHTS["transparency"],
) -> pd.DataFrame:
    """
    Return the scored dataframe with component breakdowns and final labels.
 
    Five dimensions are integrated:
      - Environmental impact  (emissions_kgco2e)
      - Ethical risk          (ethical_risk_score, 1-5)
      - Creative displacement (creative_displacement_score, 1-5)
      - Purpose justification (purpose_category: essential/beneficial/low_benefit/harmful)
      - Transparency          (transparency_score, 1-5; 1=opaque, 5=fully explainable)
    """
    required = [
        "use_case_name",
        "emissions_kgco2e",
    ...
    # Transparency: higher score = more transparent = better (not inverted)
    # Falls back to 0.5 (neutral) if column absent, so existing datasets still work
    if "transparency_score" in out.columns:
        out["transparency_norm"] = (out["transparency_score"].astype(float) - 1.0) / 4.0
    else:
        out["transparency_norm"] = 0.5
 
    weight_sum = w_emissions + w_ethics + w_creativity + w_purpose + w_transparency
    if weight_sum <= 0:
    ...
    score_0_1 = (
        ...
        + w_transparency * out["transparency_norm"]
    ) / weight_sum
    ...
    out["component_transparency"] = (100.0 * out["transparency_norm"]).round(2)
    ...
    out["contribution_transparency"] = (100.0 * (w_transparency * out["transparency_norm"] / weight_sum)).round(2)
